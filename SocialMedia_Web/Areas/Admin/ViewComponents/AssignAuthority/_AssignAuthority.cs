...
namespace SocialMedia_Web.Areas.Admin.ViewComponents.AssignAuthority
{
    public class _AssignAuthority : ViewComponent
    {
        ...
        {
            var responseMessageClaim = await _httpClientFactory.CreateClient().GetAsync("http://localhost:65525/api/OperationClaims/getall");
            if (responseMessageClaim.IsSuccessStatusCode)
            {
                ...
                var responseMessageUser = await _httpClientFactory.CreateClient().GetAsync("http://localhost:65525/api/Users/getalldto");
