...
namespace SocialMedia_Web.Areas.Admin.Controllers
{
    ...
    public class TopicController : Controller
    {
        ...
        public async Task<IActionResult> GetUpdateTopic(int id)
        {
            ...
            var responseMessage = await _httpClientFactory.CreateClient().GetAsync("http://localhost:65525/api/Topics/getbyid?id="+id);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        public async Task<IActionResult> UpdateTopic(Topics topics)
        {
            ...
            var responseMessage = await _httpClientFactory.CreateClient().PutAsync("http://localhost:65525/api/Topics/update", content);
            if (responseMessage.IsSuccessStatusCode)
            {
